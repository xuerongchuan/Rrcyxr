...
#            self.embedding_T = tf.concat([t1, t2], 0 , name='emebedding_T')
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            # self.output = tf.expand_dims(tf.reduce_sum(self.embedding_p*self.embedding_q, 1),1) + self.bias_i
    ...
    def train_and_evaluate(self):
        ...
        #                 predictions = np.apply_along_axis(lambda x: 5*(x-min_)/(max_-min_), 0,predictions)
                        ...
                        mae = sum(abs(predictions - labels_data))/len(labels_data)
                        rmse = math.sqrt(sum((predictions-labels_data)**2)/len(labels_data))
                        maes.append(mae)
                        rmses.append(rmse)
                        losses.append(test_loss)
                    mae_mean, rmse_mean, test_loss = np.array(maes).mean(), np.array(rmses).mean(), np.array(losses).mean()
                    eval_time = time.time() - eval_begin
        #             print("Epoch %d [%.1fs ]:  train_loss = %.4f" % (
        #                     epoch_count,train_time, train_loss))    
                    print("Epoch %d [ %.1fs]: mae_mean = %.4f, rmse_mean = %.4f, loss = %.4f [%.1fs] train_loss = %.4f [%.1fs]" % (
                                epoch_count, train_time, mae_mean, rmse_mean, test_loss, eval_time, train_loss, loss_time))
