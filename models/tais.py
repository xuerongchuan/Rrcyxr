    ...
    def _create_variables(self):
        with tf.name_scope('embeddings'):
            ...
            self.embedding_Q = tf.Variable(tf.truncated_normal(shape=[self.dl.num_items,self.config.embedding_size]), name='embediing_Q', \
                                     dtype=tf.float32)
            self.t1 = tf.Variable(tf.truncated_normal(shape=[self.numT, self.config.time_embedding_size], mean=0.0, stddev=0.01),\
                            name='t1', dtype = tf.float32)
            self.t2 = tf.constant(0.0, tf.float32, [1, self.config.time_embedding_size], name='t2')
            self.embedding_T = tf.concat([self.t1, self.t2], 0 , name='emebedding_T')
